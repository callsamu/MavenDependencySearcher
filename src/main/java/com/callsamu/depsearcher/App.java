...
public class App implements Callable<Integer> {
  ...
  @Option(names={"-l", "--latest"}, description = "A query parameter to be used")
  ...
  private Optional<DependencyData> find(List<DependencyData> deps, Optional<String> version) {
	  ...
	  if (version.isEmpty()) {
		  ...
		  String message = Ansi.AUTO.string( 
			  "@|bold " +
			  "Choose a version of " + 
			  first.groupId() + 
			  ":" + 
  ...
  private static Optional<DependencyData> choose(String message, List<DependencyData> deps, Scanner scanner) {
	  int index = 0;
